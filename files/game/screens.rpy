#Update subtitles configurations
init python:
    import os
    def UpdateConfig():
        filename = "game/subs.txt"
        filevar = "0"
        if subtitles == True and voices == "Spanish":
             filevar = "1"
        elif subtitles == False and voices == "Spanish":
             filevar = "2"
        elif subtitles == True and voices == "English":
             filevar = "3"
        else:
             filevar = "4"
        configFile = open(filename, 'w')
        configFile.write(filevar)
        configFile.close()
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Introduzca su nombre", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("New Game") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Introduzca su nombre", ok_action=Function(FinishEnterName)))
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Help") action [Help("README.html"), Show(screen="dialog", message="El archivo de ayuda se ha abierto en el navegador.", ok_action=Hide("dialog"))]
    ...
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="Error en el archivo: \"characters/sayori.chr\"\n\nEl archivo no existe o está corrupto.",
                ok_action=Show(screen="dialog", message="Las partidas guardadas están dañadas\nComenzando una nueva partida.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="No te molestes en guardar ya.\nDescuida, que de aquí no me voy.", ok_action=Hide("dialog"))
        else:
        ...
        vbox:
            ...
            hbox:
                vbox:
                    style_prefix "radio"
                    label _("Subtítulos")
                    textbutton _("Sí") action [SetVariable("subtitles", True), Function(UpdateConfig)]
                    textbutton _("No") action [SetVariable("subtitles", False), Function(UpdateConfig)]
                vbox:
                    style_prefix "radio"
                    label _("Voces")
