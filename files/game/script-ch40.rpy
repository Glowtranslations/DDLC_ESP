...
label ch40_main:
    ...
    python:
        if not persistent.monika_back:
            try:
                ...
                renpy.call_screen("dialog", message="Por favor, deja de jugar con mis sentimientos.\nNo quiero volver.", ok_action=Return())
    ...
    label ch40_clearnormal:
        ...
        call screen dialog("No voy a dejar que le hagas daño.", ok_action=Return())
        s "Who..."
        s "I-It hurts--"
        show screen tear(20, 0.1, 0.1, 0, 40)
        window hide(None)
        ...
        window show(None)
        s "Ah--"
        call screen dialog("Lo siento, al final estaba equivocada...", ok_action=Return())
        call screen dialog("No hay felicidad después de todo.", ok_action=Return())
        call screen dialog("Hasta siempre, Sayori.", ok_action=Return())
        call screen dialog("Hasta siempre, [player].", ok_action=Return())
        call screen dialog("Hasta siempre, club de literatura", ok_action=Return())
