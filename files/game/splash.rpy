init python:
    menu_trans_time = 1
    splash_message_default = "Este juego no es apto para menores o personas sensibles."
    splash_messages = [
    "Eres mi solecito,\nMi único solecito",
    "Te echaba de menos.",
    "Juega conmigo",
    "Esto solo es un juego,\nal menos en la mayor parte.",
    "¿Acaso este juego no es apto para menores\no personas sensibles?",
    "sdfasdklfgsdfgsgoinrfoenlvbd",
    "null",
    "He mandado a muchos niños\nal infierno.",
    "PM murió\npor esto.",
    "Esto en poca parte es tu culpa.",
    "Este juego no es apto para menores\no personas fácilmente descuartizables.",
    "Que no se te olvide hacer una copia de mi archivo."
    ]
...
label splashscreen:
    ...
    if not persistent.first_run:
        ...
        with Dissolve(1.0)
        pause 1.0
        "Este juego no es apto para menores o personas sensibles."
        "Si padeces ansiedad o depresión, te instamos a que dejes de jugar inmediatamente. Si quieres saber más, visita este enlace: https://tinyurl.com/ddlcglow"
        menu:
    ...
    if s_kill_early:
        ...
        show noise:
            alpha 0.1
        with Dissolve(1.0)
        show expression Text("Ahora todas serán felices.", style="sayori_text"):
            xalign 0.8
            yalign 0.5
            alpha 0.0
            600
...
label after_load:
    ...
    if persistent.yuri_kill > 0 and persistent.autoload == "yuri_kill_2":
        ...
        if persistent.playthrough == 0 and not persistent.first_load and not config.developer:
            ...
            call screen dialog("Pista: Puedes usar el botón «Saltar» para\nsaltarte el texto que ya hayas visto.", ok_action=Return())
    return
